...
def find_transforms(pose, depth_val) :
    ...
    for i in range(len(pose)) :
        ...
        p.pose.position.x= X + 0.15
        p.pose.position.y= Y - 0.15
        p.pose.position.z= Z + 0.5
        p.pose.orientation.x=0
        p.pose.orientation.y=0
        p.pose.orientation.z=0
        p.pose.orientation.w=0

        res=tf_buffer.transform(p,'odom',timeout=rospy.Duration(5))
...
def main():
    ...
    inter_pose2 =  [math.radians(90),math.radians(17),math.radians(-25),math.radians(0),math.radians(0),math.radians(0)]
