...
def transform_pose():
    ...
    try:
        ...
        trans = tf_buffer.lookup_transform('pepper' , 'ebot_base' , rospy.Time(0))
        return trans
...
def find_transforms(pose, depth_val) :
    ...
    for i in range(len(pose)) :
        ...
        p1.header.frame_id='camera_depth_frame2'
        p1.pose.position.x= X - 0.06
        p1.pose.position.y= Y + 0.25
        p1.pose.position.z= Z + 0.60
        p1.pose.orientation.x=0
        p1.pose.orientation.y=0
        p1.pose.orientation.z=0
        p1.pose.orientation.w=0

        res=tf_buffer.transform(p1,'ebot_base',timeout=rospy.Duration(5))
...
def main():
    ...
    # ur5 = Ur5Moveit()
    # ps = PerceptionStack() 
    print("printing the trans value")
