...
class PercepStack():
    def __init__(self) -> None:
        ...
        self.found=False
        self.rotated_side_arm = rospy.Subscriber("rotated_side_arm" , String , self.rotated_side_arm_clbk)
        self.publish_frame = 0

    def rotated_side_arm_clbk(self,msg):
        msg = msg.data
        try :
            if msg == "Side_pose_done":
                self.publish_frame = 1
            else :
                self.publish_frame = 0
        except :
            print("No arm rotated")
    ...
    def callback(self,depth_data, rgb_data) :
        ...
        if self.found and self.publish_frame == 1 :
